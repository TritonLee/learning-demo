...
for imageNum = 1:numImages
  for filterNum = 1:numFilters
    ...
    convolvedImage = conv2(im,filter,'valid'); %利用光计算完成
    % Add the bias unit
    % activation function: sigmoid function
    % Then, apply the sigmoid function to get the hidden activation    
    convolvedImage = bsxfun(@plus,convolvedImage,b(filterNum)); % add bias
    convolvedImage = 1./(1+exp(-convolvedImage));      % activation function
